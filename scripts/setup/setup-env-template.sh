...
DEFAULT_TEMPLATE_DIR="$ROOT_DIR/infra/env/templates"
DEFAULT_OUTPUT_FILE="$ROOT_DIR/.env"
...
collect_placeholders() {
  ...
  grep -vE '^[[:space:]]*#' "$template_file" 2>/dev/null \
...
render_template() {
  ...
  # Di env setup, kita bisa ignore error missing variables untuk kelenturan atau membiarkanya error. 
  # Lebih baik biarkan error agar fill var benar.
  if [ "${#missing[@]}" -gt 0 ]; then
    if [ "$MODE" = "replace-file" ]; then
      echo -e "${YELLOW}[WARN]${NC} Missing variables for $(template_id "$template_file"): ${missing[*]}" >&2
      echo -e "${YELLOW}[WARN]${NC} Keeping unresolved placeholders in output because -f was used." >&2
    else
      echo -e "${RED}[ERROR]${NC} Missing variables for $(template_id "$template_file"): ${missing[*]}" >&2
      exit 1
    fi
  fi
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    if [[ "$line" =~ ^([A-Za-z_][A-Za-z0-9_]*)=(.*)$ ]]; then
      ...
      if grep -qE "^${key}=" "$target_file" 2>/dev/null; then
        echo -e "${YELLOW}[INFO]${NC} Key ${YELLOW}${key}${NC} already exists in target, replaced value."
        
        local awk_tmp
        awk_tmp="$(mktemp)"
        
        __AWK_VAL="$val" awk -v k="$key" '
          BEGIN { 
            v = ENVIRON["__AWK_VAL"]
          }
          $0 ~ "^" k "=" { 
...
write_blocks() {
  ...
  if [ "$MODE" = "replace-file" ]; then
    ...
    for index in "${!RENDERED_BLOCKS[@]}"; do
      ...
      inject_block_with_dedup "$temp_output" "${RENDERED_BLOCKS[$index]}"
      first_block=0
    done
  ...
  for index in "${!RENDERED_BLOCKS[@]}"; do
    ...
    inject_block_with_dedup "$OUTPUT_FILE" "${RENDERED_BLOCKS[$index]}"
    echo -e "${GREEN}[OK]${NC} Appended template block ${id} into $OUTPUT_FILE"
  done
}

# DO NOT LOAD .env or .env.backend implicitly in env template renderer!
# It will load values from outputs instead of inputs!
# load_env "$ROOT_DIR/.env" -> Removed so it strictly uses -E
